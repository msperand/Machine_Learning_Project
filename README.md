...
## The Streamlit
A popular way to learn a language is by listening to music in that language. Many people use this technique to learn French, but one common challenge is knowing which songs to choose. If the song is too simple, the benefits are minimal, but if the song is too complex, it can be overwhelming. To address this issue, we developed 'SongEase,' an app designed to help French learners select songs that match their proficiency level. Our app estimates the overall difficulty level of a French song (A1, A2, B1, B2, C1, C2) and assesses the difficulty of each sentence in the song. 

'SongEase' comes with a selection of preloaded songs, ranging from French classics to contemporary hits. Additionally, the app allows users to test the difficulty of their own song choices.
